...
namespace OpenWorldMapCreatorSystem {
    ...
    inline void create_forest(Registry& registry, const glm::vec2& center_position, int num_trees = 500, float min_distance = 8.0f) {
        ...
        while (trees_created < num_trees && attempts > 0) {
            ...
            if (is_position_valid(new_pos, tree_positions, min_distance)) {
                EntityFactory::create_tree(registry, new_pos);
